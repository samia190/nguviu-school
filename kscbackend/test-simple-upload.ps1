...
Write-Host "Testing Image Upload Flow`n" -ForegroundColor Cyan
...
try {
    ...
    Write-Host "Step 3 - Sending to /api/home-news..." -ForegroundColor Yellow
    ...
    if ($newsData._id) {
        ...
        # Step 4: Fetch back
        Write-Host "Step 4 - Fetching back from /api/home-news/:id..." -ForegroundColor Yellow
        ...
        Write-Host "2. Stored in DB: $(if ($newsData.imageUrl.StartsWith('http')) { 'ABSOLUTE' } else { 'RELATIVE' })"
        Write-Host "   URL: $($newsData.imageUrl)"
        Write-Host "3. Fetched from DB: $(if ($fetchData.imageUrl.StartsWith('http')) { 'ABSOLUTE' } else { 'RELATIVE' })`n"
        
        if ($uploadedUrl.StartsWith('http') -and -not $newsData.imageUrl.StartsWith('http')) {
            Write-Host "WARNING: Image URL lost absolute path when saving to database!" -ForegroundColor Red
        }
    }
