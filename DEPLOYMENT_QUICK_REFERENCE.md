...
# Frontend accessible
curl https://kangarugirls.sc.ke/ | grep "DOCTYPE"
```
...
fetch('https://kangarugirls.sc.ke/api/home-news?active=true')
  .then(r => r.json())
...
3. Verify it contains `https://kangarugirls.sc.ke`
...
- [ ] `curl https://kangarugirls.sc.ke/api/health` returns `{"status":"ok"}`
